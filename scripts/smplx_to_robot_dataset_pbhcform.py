...
def gmr_to_pbhcform(root_pos,
    ...
    if num_dof == 29:
        ...
        dof = np.concatenate((dof_pos[:, :19], dof_pos[:, 22:26]), axis=1)
    else:
...
def process_file(smplx_file_path,
                 tgt_file_path,
                 tgt_robot,
                 SMPLX_FOLDER,
                 tgt_folder,
    ...
    while check_memory():
        print(
            f"[PAUSE] Paused processing {smplx_file_path} to prevent memory overflow. num_pause: {num_pause}"
        )
        time.sleep(60 * 2)
        num_pause += 1
        if num_pause > 10:
            print(
                f"[ERROR] Memory usage is still high after 10 pauses. Exiting."
            )
            return
    ...
    if HEIGHT_ADJUST:
        ...
        body_pos, _ = kinematics_model.forward_kinematics(
            ...
            torch.from_numpy(dof_pos).to(device=device,
                                         dtype=torch.float))  # TxNx3
        ground_offset = 0.0
        lowerst_height = torch.min(body_pos[..., 2]).item()
        root_pos[:,
                 2] = root_pos[:,
                               2] - lowerst_height + ground_offset  # make sure motion on the ground
...
def main():
    ...
    hard_motions_paths = [
        hard_motions_folder / "0.txt", hard_motions_folder / "1.txt"
    ]
    hard_motions = []
    ...
    for dirpath, _, filenames in os.walk(src_folder):
        for filename in natsorted(filenames):
            ...
            if filename.endswith((".pkl", ".npz")):
                ...
                tgt_file_path = smplx_file_path.replace(
                    src_folder, tgt_folder).replace(".npz", ".pkl")
                if not os.path.exists(tgt_file_path) or args.override:
                    args_list.append((smplx_file_path, tgt_file_path,
                                      args.robot, SMPLX_FOLDER, tgt_folder))
    ...
    with mp.Pool(args.num_cpus) as pool:
        pool.starmap(process_file,
                     [args + (total_files, verbose) for args in args_list])
