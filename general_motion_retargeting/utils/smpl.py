...
def load_smplx_file(smplx_file, smplx_body_model_path):
    ...
    smplx_output = body_model(
        ...
        global_orient=torch.tensor(
            smplx_data["root_orient"]).float(),  # (N, 3)
...
def get_smplx_data_offline_fast(smplx_data,
                                body_model,
                                smplx_output,
                                tgt_fps=30):
    """
    ...
    frame_skip = float(src_fps / tgt_fps)
    num_frames = smplx_data["pose_body"].shape[0]
    ...
    if tgt_fps != src_fps:
        ...
        new_num_frames = int(num_frames / frame_skip)
        ...
        for i in range(joints.shape[1]):  # For each joint
            for j in range(3):  # For each coordinate
                interp_func = interp1d(original_time,
                                       joints[:, i, j],
                                       kind='linear')
    ...
    for curr_frame in range(len(global_orient)):
        ...
        for i, joint_name in enumerate(joint_names):
            ...
            joint_orientations.append(rot)
            result[joint_name] = (single_joints[i],
...
def get_gvhmr_data_offline_fast(smplx_data,
                                body_model,
                                smplx_output,
                                tgt_fps=30):
    """
