...
class GeneralMotionRetargeting:
    ...
    def __init__(
        ...
        if verbose:
            print(
                "[GMR] Robot Degrees of Freedom (DoF) names and their order:")
        ...
        for i in range(
                self.model.nu):  # 'nu' is the number of actuators (motors)
        ...
        if use_velocity_limit:
            VELOCITY_LIMITS = {
                k: 3 * np.pi
                for k in self.robot_motor_names.keys()
            }
    ...
    def setup_retarget_configuration(self):
        ...
        for frame_name, entry in self.ik_match_table1.items():
            ...
            if pos_weight != 0 or rot_weight != 0:
                ...
                self.pos_offsets1[body_name] = np.array(
        ...
        for frame_name, entry in self.ik_match_table2.items():
            ...
            if pos_weight != 0 or rot_weight != 0:
                ...
                self.pos_offsets2[body_name] = np.array(
                    pos_offset) - self.ground
    ...
    def update_targets(self, human_data, offset_to_ground=False):
        ...
        human_data = self.scale_human_data(human_data, self.human_root_name,
                                           self.human_scale_table)
        human_data = self.offset_human_data(human_data, self.pos_offsets1,
                                            self.rot_offsets1)
    ...
    def retarget(self, human_data, offset_to_ground=False):
        ...
        if self.use_ik_match_table2:
            ...
            dt = self.configuration.model.opt.timestep
            vel2 = mink.solve_ik(self.configuration, self.tasks2, dt,
                                 self.solver, self.damping, self.ik_limits)
        ...
        for body_name in human_data.keys():
            ...
            if body_name == human_root_name:
                ...
                human_data_local[body_name] = (
                    human_data[body_name][0] -
                    root_pos) * human_scale_table[body_name]

        # transform the human data back to the global frame
        human_data_global = {human_root_name: (scaled_root_pos, root_quat)}
        for body_name in human_data_local.keys():
            human_data_global[body_name] = (human_data_local[body_name] +
                                            scaled_root_pos,
    ...
    def offset_human_data(self, human_data, pos_offsets, rot_offsets):
        ...
        for body_name in human_data.keys():
            ...
            global_pos_offset = R.from_quat(
                updated_quat, scalar_first=True).apply(local_offset)
    ...
    def offset_human_data_to_ground(self, human_data):
        ...
        for body_name in human_data.keys():
            pos, quat = human_data[body_name]
            offset_human_data[body_name] = [pos, quat]
            offset_human_data[body_name][0] = pos - np.array(
                [0, 0, lowest_pos]) + np.array([0, 0, ground_offset])
        return offset_human_data
    ...
    def apply_ground_offset(self, human_data):
        for body_name in human_data.keys():
            pos, quat = human_data[body_name]
            human_data[body_name][0] = pos - np.array(
                [0, 0, self.ground_offset])
        return human_data
